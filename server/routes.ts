...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Create new user (generic endpoint for all user types)
  app.post("/api/users", isOwner, async (req, res) => {
    try {
      // Validate the role
      if (!["employee", "customer"].includes(req.body.role)) {
        return res
          .status(400)
          .json({ error: "Invalid role. Must be 'employee' or 'customer'" });
      }
  ...
  // Create new employee (legacy endpoint)
  ...
  app.post("/api/distributions", isOwner, async (req, res) => {
    try {
      ...
      if (existingStock) {
        await storage.updateEmployeeStock(existingStock.id, {
  ...
  app.post("/api/sales", isEmployee, async (req, res) => {
    try {
      ...
      await storage.updateEmployeeStock(employeeStock.id, {
